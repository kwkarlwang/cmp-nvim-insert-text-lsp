local source = require("cmp_nvim_insert_text_lsp.source")
...
M.setup = function()
	vim.api.nvim_create_autocmd("InsertEnter", {
		group = vim.api.nvim_create_augroup("cmp_nvim_insert_text_lsp", { clear = true }),
		pattern = "*",
	...
	return {
		textDocument = {
			completion = {
				...
				completionItem = {
					snippetSupport = if_nil(override.snippetSupport, false),
...
M.update_capabilities = function(_, override)
	...
	_deprecate(
		"cmp_nvim_insert_text_lsp.update_capabilities",
		"cmp_nvim_insert_text_lsp.default_capabilities",
		"1.0.0",
	...
	for _, client in ipairs(vim.lsp.get_active_clients()) do
		...
		if not M.client_source_map[client.id] then
			...
			if s:is_available() then
				M.client_source_map[client.id] = cmp.register_source("nvim_insert_text_lsp", s)
			end
		end
	end
